create or replace view "tn_release"."v_ust_release_substance" as
 SELECT DISTINCT (r.release_id)::character varying(50) AS release_id,
  ...
  WHERE ((b.epa_value IS NOT NULL) AND (a."Currentstatus" <> ALL (ARRAY['0a Suspected Release - Closed'::text, '0 Suspected Release - RD records'::text, '3 Release Investigation'::text])) AND (a."Discoverydate" IS NOT NULL));