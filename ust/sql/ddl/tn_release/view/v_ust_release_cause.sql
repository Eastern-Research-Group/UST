create or replace view "tn_release"."v_ust_release_cause" as
 SELECT DISTINCT (r.release_id)::character varying(50) AS release_id,
   ...
   FROM ((tn_release."ust_all-tn-environmental-sites" x
     ...
     LEFT JOIN tn_release.v_cause_xwalk rc ON ((x."Cause" = (rc.organization_value)::text)))
  WHERE ((rc.epa_value IS NOT NULL) AND (x."Currentstatus" <> ALL (ARRAY['0a Suspected Release - Closed'::text, '0 Suspected Release - RD records'::text, '3 Release Investigation'::text])));