...
CREATE OR REPLACE FUNCTION public.get_template_element_tabs(v_element_id integer, ust_or_release character varying DEFAULT 'ust'::character varying)
 RETURNS text
 LANGUAGE plpgsql
AS $function$
declare 
...
begin 
	if ust_or_release = 'ust' then
		...
		from 
			(select distinct b.template_tab_name as tab,
				b.sort_order
			from release_elements_tables a join release_element_table_sort_order b 
				on a.table_name  = b.table_name
		where a.element_id = v_element_id) x;	
	end if;
	return v_tabs;
