create or replace view "ca_release"."v_lust_geocode" as
 SELECT DISTINCT lust_old."FacilityID",
  ...
  WHERE (((lust_old.organization_id)::text = 'CA'::text) AND (lust_old.control_id = ( SELECT max(lust_control.control_id) AS max
           FROM archive.lust_control
          WHERE ((lust_control.organization_id)::text = 'CA'::text))));