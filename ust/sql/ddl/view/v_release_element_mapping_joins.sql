create or replace view "public"."v_release_element_mapping_joins" as
 SELECT a.epa_table_name,
   ...
   FROM (((release_element_mapping a
     LEFT JOIN release_element_table_sort_order b ON (((a.epa_table_name)::text = (b.table_name)::text)))
     LEFT JOIN release_elements c ON (((a.epa_column_name)::text = (c.database_column_name)::text)))
     LEFT JOIN release_elements_tables d ON (((c.element_id = d.element_id) AND ((b.table_name)::text = (d.table_name)::text))));