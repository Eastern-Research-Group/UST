...
/*********** v_ust_tank ***********/
--Unable to complete peer review row-count comparison for dc_ust.v_ust_tank.
--The database connection closed while running this SQL; run it manually to diagnose the view performance or definition.

select count(*) from dc_ust.v_ust_tank;



/*********** v_ust_tank_substance ***********/
--Unable to complete peer review row-count comparison for dc_ust.v_ust_tank_substance.
--The database connection closed while running this SQL; run it manually to diagnose the view performance or definition.

select count(*) from dc_ust.v_ust_tank_substance;



/*********** v_ust_facility ***********/
--There are 28 rows in dc_ust.v_ust_facility that do not exist in public.v_ust_facility

select * from dc_ust.v_ust_facility a
where not exists
	(select 1 from public.v_ust_facility b
	where a.facility_id = b."FacilityID")
order by a.facility_id;

--View definition for dc_ust.v_ust_facility:
 SELECT DISTINCT (x."FACILITYID")::character varying(50) AS facility_id,
    x."FacilityName" AS facility_name,
        CASE
            WHEN ((x."OwnerType" = 'Federal Government - Non Military'::text) AND (x."FinancialResponsibilitySelfInsuranceFinancialTest" = 'Yes'::text)) THEN NULL::integer
            ELSE ot.owner_type_id
    ...
    x."FacilityOperatorCompanyName" AS facility_operator_company_name,
        CASE
            WHEN ((x."OwnerType" = 'Federal Government - Non Military'::text) AND (x."FinancialResponsibilitySelfInsuranceFinancialTest" = 'Yes'::text)) THEN 'N/A'::text
    ...
    x."FinancialResponsibilityRiskRetentionGroup" AS financial_responsibility_risk_retention_group,
        CASE
            WHEN ((x."OwnerType" = 'Federal Government - Non Military'::text) AND (x."FinancialResponsibilitySelfInsuranceFinancialTest" = 'Yes'::text)) THEN NULL::text
            ELSE x."FinancialResponsibilitySelfInsuranceFinancialTest"
...
/*********** v_ust_compartment ***********/
--There are 62 rows in dc_ust.v_ust_compartment that do not exist in public.v_ust_compartment

select * from dc_ust.v_ust_compartment a
where not exists
...
--View definition for dc_ust.v_ust_compartment:
...
--There are 62 rows in dc_ust.v_ust_compartment_substance that do not exist in public.v_ust_compartment_substance

select * from dc_ust.v_ust_compartment_substance a
where not exists
...
--View definition for dc_ust.v_ust_compartment_substance:
 SELECT DISTINCT (x."FacilityID")::text AS facility_id,
...
/*********** v_ust_piping ***********/
--Unable to count rows in dc_ust.v_ust_piping that do not exist in public.v_ust_piping; run the SQL below manually.

select * from dc_ust.v_ust_piping a
where not exists
...
--View definition for dc_ust.v_ust_piping:
 SELECT DISTINCT (x."FacilityID")::text AS facility_id,
    (t."TankName")::integer AS tank_id,
...
/*********** v_ust_tank ***********/
--There are 62 rows in dc_ust.v_ust_tank that do not exist in public.v_ust_tank

select * from dc_ust.v_ust_tank a
where not exists
	(select 1 from public.v_ust_tank b
	where a.facility_id = b."FacilityID" and a.tank_id = b."TankID")
order by a.facility_id,a.tank_id;

--View definition for dc_ust.v_ust_tank:
 SELECT DISTINCT (x."FacilityID")::text AS facility_id,
...
/*********** v_ust_tank_substance ***********/
--There are 62 rows in dc_ust.v_ust_tank_substance that do not exist in public.v_ust_tank_substance

select * from dc_ust.v_ust_tank_substance a
where not exists
...
--View definition for dc_ust.v_ust_tank_substance:
 SELECT DISTINCT (x."FacilityID")::text AS facility_id,
    (t."TankName")::integer AS tank_id,
    s.substance_id,
