...
create or replace view gu_ust.v_ust_facility as
 SELECT DISTINCT x."Permit Number" AS facility_id,
...
create or replace view gu_ust.v_ust_tank as
 ...
 where not exists
	(select 1 from gu_ust.erg_unregulated_tanks unreg
	where x."FacilityID"::varchar(50) = unreg.facility_id and x."TankID"::int = unreg.tank_id);



create or replace view gu_ust.v_ust_tank_substance as
 SELECT DISTINCT x."FacilityID" AS facility_id,
    x."TankID" AS tank_id,
        CASE
...
create or replace view gu_ust.v_ust_compartment as
 SELECT DISTINCT x."FacilityID" AS facility_id,
    (x."TankID")::integer AS tank_id,
    c.compartment_id,
        CASE
...
create or replace view gu_ust.v_ust_compartment_substance as
 SELECT DISTINCT x."FacilityID" AS facility_id,
    x."TankID" AS tank_id,
    c.compartment_id,
        CASE
...
create or replace view gu_ust.v_ust_piping as
 SELECT DISTINCT x."FacilityID" AS facility_id,
    (x."TankID")::integer AS tank_id,
    c.compartment_id,
