...
class Summary:
    ...
    def export_summary(self):
        ...
        for rowno, row in enumerate(data, start=1):
            for colno, cell_value in enumerate(row, start=1):
                ...
                if colno == 2 and ws.cell(row=rowno, column=colno-1).value == 'organization_compartment_flag' and ws.cell(row=rowno, column=colno).value is None:
    ...
    def performance_measure_comparison(self):
        ...
        if self.dataset.ust_or_release == 'ust':
            ...
            sql = """select total_type, total_cumulative_releases
            ...
            ws.cell(row=rowno, column=2).font = Font(bold=True)
            sql = """select "USTReleaseStatus", count(*) from public.v_ust_release
