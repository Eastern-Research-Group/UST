...
def main(control_id, ust_or_release, table_name, column_name, delimiter=',', drop_existing=False):
	...
	sql = f"""update public.{ust_or_release}_element_mapping 
			  set deagg_table_name = %s, deagg_column_name = %s
	          where {ust_or_release}_control_id = %s 
	          and organization_table_name = %s and organzation_column_name = %s"""
	cur.execute(sql, (deagg_table_name, id_column_name, control_id, table_name, column_name))
