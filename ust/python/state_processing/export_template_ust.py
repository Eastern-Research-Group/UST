...
class Template:
	...
	def __init__(self, 
		...
		self.ust_control_id = ust_control_id
	...
	def print_self(self):
		...
		print('ust_control_id = ' + str(self.ust_control_id))
	...
	def populate_org_control(self):
		...
		cur = conn.cursor()	
		if self.template_only and not self.organization_id and not self.ust_control_id and not self.export_file_path and not self.export_file_dir and not self.export_file_name:
			logger.error('If you want to export a template only and are not passing an organization_id or ust_control_id, you must pass export_file_name and export_file_dir OR export_file_path. Exiting...')
			exit()
		elif self.template_only and not (self.export_file_path or (self.export_file_dir and self.export_file_name)) and (self.organization_id or self.ust_control_id):
			logger.warning("You requested a template only but didn't pass an export path or directory/file name. The file name and path will be constructed from the organization_id/ust_control_id passed (%s/%s)", self.organization_id, self.ust_control_id)
			...
			if self.export_file_path or (self.export_file_dir and self.export_file_name):
				...
				exit()
		elif not self.organization_id and not self.ust_control_id:
			logger.error("If you don't want a template only, either organization_id or ust_control_id must be passed. Exiting...")
			exit()
		elif not self.ust_control_id:
			sql = "select max(ust_control_id) from ust_control where organization_id = %s"
			cur.execute(sql, (self.organization_id,))
			self.ust_control_id = cur.fetchone()[0]
		else:
			sql = "select organization_id from ust_control where ust_control_id = %s"
			cur.execute(sql, (self.ust_control_id,))
			org_id = cur.fetchone()[0]
			if self.organization_id and org_id != self.organization_id:
				logger.warning('ust_control_id %s is %s, but %s was passed. Exiting.', self.ust_control_id, org_id, self.organization_id)
				exit()
			self.organization_id = org_id
		cur.close()
		conn.close()
		logger.debug('ust_control_id = %s, organization_id = %s', self.ust_control_id, self.organization_id)
	...
	def make_reference_tab(self):
		...
		headers = utils.get_headers('v_ust_elements')
		for colno, header in enumerate(headers, start=1):
			ws.cell(row=1, column=colno).value = header
		conn = utils.connect_db()
		cur = conn.cursor()
		sql = "select * from public.v_ust_elements"	
		cur.execute(sql)
		data = cur.fetchall()
		for rowno, row in enumerate(data, start=2):
			for colno, cell_value in enumerate(row, start=1):
				if colno == 8 and cell_value:
	...
	def make_lookup_tabs(self):
		lookups = utils.get_lookup_tabs(ust_or_release='ust')
	...
	def make_mapping_tab(self, mapping):
		...
		sql = f"""select distinct organization_value, epa_value, programmer_comments, epa_comments, organization_comments
				...
				where ust_control_id = %s and epa_column_name = %s
				order by 1, 2"""
		cur.execute(sql, (self.ust_control_id, mapping_column_name))
	...
	def make_substance_mapping_tab(self):
		...
		sql = f"""select distinct organization_value, epa_value, programmer_comments, epa_comments, organization_comments
				from public.v_ust_element_mapping 
				where ust_control_id = %s and epa_column_name = 'substance'
				order by 1, 2"""
		cur.execute(sql, (self.ust_control_id,))
	...
	def make_data_tabs(self):
		tabs = utils.get_data_tabs(ust_or_release='ust')
		for tab in tabs:
	...
	def make_data_tab(self, tab):
		...
		if not self.template_only:
			conn = utils.connect_db()
			cur = conn.cursor()
			sql = f"select * from public.{view_name} where ust_control_id = %s"
			cur.execute(sql, (self.ust_control_id,))
			data = cur.fetchall()
			for rowno, row in enumerate(data, start=2):
				for colno, cell_value in enumerate(row, start=1):
...
def main(organization_id=None, ust_control_id=None, data_only=False, template_only=False, export_file_name=None, export_file_dir=None, export_file_path=None):
	template = Template(organization_id=organization_id, 
