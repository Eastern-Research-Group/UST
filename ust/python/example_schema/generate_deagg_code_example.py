	...
	def generate_sql(self):
		...
		for row in rows:
			...
			if deagg_flag:
				...
				self.deagg_sql = self.deagg_sql + ' * run deagg_example.py, setting the variables below.\n'
				self.deagg_sql = self.deagg_sql + ' * Setting variable deagg_rows to the default value of True will run deagg_rows_example.py after running\n'
				self.deagg_sql = self.deagg_sql + ' * deagg_example.py, which is usually the behavior you will want.\n'
				self.deagg_sql = self.deagg_sql + f' * Both scripts will automatically update example.{self.dataset.ust_or_release}_element_mapping to to map the new deagg table(s).\n\n'
				...
				# self.deagg_sql = self.deagg_sql + '* After running deagg_example.py, run the SQL below to view the value deagg table:\n*/\n'
				# self.deagg_sql = self.deagg_sql + f'select * from {self.dataset.schema}.{deagg_table_name} order by 2;\n\n'
				# self.deagg_sql = self.deagg_sql + '/* Next, run script deagg_rows_example.py to crosswalk the deaggreated value to facility, tank, or compartment-level rows.\n'
				...
				# script_params = script_params + f"deagg_table_name = '{deagg_table_name}'           # Deagg table name generated by deagg_example.py. It will begin with an 'erg_' prefix. Check column deagg_table_name in table ust_element_mapping or release_element_mapping if you don't know it. (deagg_example.py will set this value.)\n"
				...
				# self.deagg_sql = self.deagg_sql + script_params + '\n\n'
				# self.deagg_sql = self.deagg_sql + ' * After running deagg_rows_example.py, run the SQL below to view the rows deagg table:\n */\n'
